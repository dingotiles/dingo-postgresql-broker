...
func (cluster *Cluster) AddNode(node Node) (kvIndex uint64, err error) {
	key := fmt.Sprintf("/serviceinstances/%s/nodes/%s/backend", cluster.Data.InstanceID, node.Id)
...
func (cluster *Cluster) RemoveNode(nodeId string) error {
	key := fmt.Sprintf("/serviceinstances/%s/nodes/%s", cluster.Data.InstanceID, nodeId)
