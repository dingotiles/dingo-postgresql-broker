...
func (lc *LicenseCheck) ServicePlanUsage(planGUID string) (count int, err error) {
	...
	if err != nil {
		return 0, fmt.Errorf("Error constructing CF client: %v", err)
	}
	req := cfclient.NewRequest("GET", fmt.Sprintf("/v2/service_plans/%s/service_instances", planGUID))
	resp, err := cfclient.DoRequest(req)
	if err != nil {
		return 0, fmt.Errorf("Error requesting service plan/service instances: %v", err)
	}
	resBody, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return 0, fmt.Errorf("Error reading service plan/service instances body: %v", err)
	}

	err = json.Unmarshal(resBody, &spuResp)
	if err != nil {
		return 0, fmt.Errorf("Error unmarshaling service plan/service instances %v", err)
	}
